...
abstract class Cache
{
  ...
  public function clear($wpdb)
  {
    ...
    if ($wpdb->last_error || empty($queryResult) || !\is_array($queryResult)) {
      \ytp_info(__FILE__, __LINE__, "DB Query empty or failed, not clearing cache. ERROR: '" . $wpdb->last_error . "'");
      return FALSE;
    }
    ...
    $success = TRUE;
    \ytp_debug(__FILE__, __LINE__, "Clearing $count cache items, triggered by user.");
  ...
  protected function logRequestMasked($url)
  {
    ...
    \ytp_debug(__FILE__, __LINE__, "No cache present, getting new data from: '$masked_url'");
  }
}
