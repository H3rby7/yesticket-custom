...
function getYesTicketEventsList($atts) {
	...
	if (count($result) > 0 && $result->message != "no items found"){
		...
		foreach($result as $item){
			...
			if ($att["type"]=="all") $add = "<br><span class='yt-eventtype'>".$item->event_type."</span>";
			$content .= "<span class='yt-eventdate'>".date('d.m.y H:i', strtotime($item->event_datetime))." Uhr</span>".$add."</span><br>";
			$content .= "<span class='yt-eventname'>".htmlentities($item->event_name)."</span>";

			$content .= "<span class='yt-eventdate'>".htmlentities($item->location_name).", ".htmlentities($item->location_city)."</span>";
			if ($att["ticketlink"]=="yes") $content .= '<br><a href="'.$item->yesticket_booking_url.'" target="_blank">Tickets</a>';
			$content .= "</div>\n";
			$count++;
			if ($count == (int)$att["count"]) break;
		}
...
function getYesTicketTestimonials($atts) {
	...
	if (empty($att["count"])) $att["count"] = 3;
	if ($att["type"]=="Impro-Show")  $att["type"] = "performance";
	if ($att["type"]=="Workshop")  $att["type"] = "workshop";
	if ($att["type"]=="Festival")  $att["type"] = "festival";
	if ($att["type"]!="all" AND $att["type"]!="performance" AND $att["type"]!="workshop" AND $att["type"]!="festival") return "FEHLER: Bitte gib ein korrektes Type-Attribut an. Gültig sind nur all, performance, workshop und festival. Wenn Du alle Events möchtest gib das Attribut einfach nicht an";
	...
	if (count($result) > 0 && $result->message != "no items found"){
		foreach($result as $item){
			...
			if (!empty($item->event_name) && $att["details"] == "yes") $add_event = "<br><span class='yt-testimonial-source'>über ".htmlentities($item->event_name)."</span>";
...
function pluginpage_init(){
        ...
        echo "<p>Du benötigst für deine Testimonials die gleichen 2 Dinge wie oben: deine persönliche <b>Organizer-ID</b> und deinen dazugehörigen <b>Key</b>. Beides findest du direkt zum Kopieren im Adminbereich von YesTicket > Mehr können > YesTicket einfach einbinden: <a href='https://www.yesticket.org/login/de/integration.php#wp-plugin' target='_blank'>https://www.yesticket.org/login/de/integration.php#wp-plugin</a></p>";
